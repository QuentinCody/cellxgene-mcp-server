...
console.log(`${BLUE}🧪 CELLxGENE Structured Content Regression Tests${RESET}`);

const toolExpectations = [
  {
    path: 'src/tools/code-mode.ts',
    required: ['createSearchTool', 'createExecuteTool', 'cellxgeneCatalog'],
  },
  {
    path: 'src/tools/query-data.ts',
    required: ['createQueryDataHandler', 'cellxgene_query_data', 'CELLXGENE_DATA_DO'],
  },
  {
    path: 'src/tools/get-schema.ts',
    required: ['createGetSchemaHandler', 'cellxgene_get_schema', 'CELLXGENE_DATA_DO'],
  },
  {
    path: 'src/spec/catalog.ts',
    required: ['cellxgeneCatalog', 'api.cellxgene.cziscience.com'],
  },
];
...
assertContains('src/index.ts', indexContent, 'CellxgeneDataDO', 'index.ts exports CellxgeneDataDO');
...
console.log(`\n${GREEN}✅ CELLxGENE structured content regression tests passed.${RESET}`);
