...
interface QueryEnv {
    CELLXGENE_DATA_DO?: unknown;
}

export function registerQueryData(server: McpServer, env?: QueryEnv): void {
    const handler = createQueryDataHandler("CELLXGENE_DATA_DO", "cellxgene");

    server.registerTool(
        "cellxgene_query_data",
        {
            title: "Query Staged CELLxGENE Data",
            description:
                "Query staged CELLxGENE Discover data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
